...
function ensureContrast(bgColor, fgColor) {
    ...
    if (diff < 128) {
        return `#${(0xFFFFFF ^ parseInt(fgColor.slice(1), 16)).toString(16).padStart(6, '0')}`;
    }
    return fgColor;
}
...
function generateCaptcha(settings) {
    ...
    const captchaText = randomText(settings.captcha.length, settings.text.characters);

    const bgColor = randomColor(settings.colors.background);
    const noiseColor = randomColor(settings.colors.noise);
    const strokeColors = settings.colors.strokes.map(color => randomColor(color));

    ctx.fillStyle = bgColor;
    ctx.fillRect(0, 0, settings.canvas.width, settings.canvas.height);

    ctx.fillStyle = noiseColor;
    for (let i = 0; i < settings.canvas.width * settings.canvas.height * settings.noise.density; i++) {
        ctx.fillRect(
